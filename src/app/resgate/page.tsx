...
export default function ResgatePage() {
  ...
  async function finalizar(dorSelecionada: string) {
    ...
    // UTMs da URL (bio/ads) — o LeverForms guarda junto com a resposta
    ...
    // Envia pro LeverForms via proxy local (server-to-server, sem CORS).
    // O LeverForms grava a resposta (painel de Leads) e dispara o webhook do
    // CLINT — a integração vive lá agora, não mais neste arquivo.
    let ok = false;
    try {
